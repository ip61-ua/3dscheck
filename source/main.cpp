...
const char *
textGetString (StrId id)
{
  const char *str = g_strings[id][language];
  if (!str)
    str = g_strings[id][CFG_LANGUAGE_EN];
  return str;
}
...
sceneRenderTop ()
{
  ...
  sprintf (timeString, "%02d:%02d:%02d", hour, min,
           seconds); // --------------------------------- //
  ...
  C2D_DrawText (&texts[6], C2D_WithColor, 2, 2, 0, 0.5f, 0.5f,
                white); // 3DSCheck string
  C2D_DrawText (&ver, C2D_WithColor | C2D_AlignRight | C2D_AtBaseline, 398,
                238, 0, 0.5f, 0.5f, white); // Language string

  m_useColor (0x37, 0x67,
              0x70); // switch back to normal color regardless of battery
  ...
  C2D_DrawText (&texts[14], C2D_WithColor | C2D_AlignRight, 385, 2, 0, 0.5f,
                0.5f, white);
  C2D_DrawText (&texts[15], C2D_WithColor | C2D_AlignRight, 398, 2, 0, 0.5f,
                0.5f, white);

  C2D_TextParse (&sysTime, timeBuf, timeString); // draw time
  C2D_TextOptimize (&sysTime);
  C2D_DrawText (&sysTime, C2D_WithColor | C2D_AlignCenter, 200, 2, 0, 0.5f,
                0.5f, white);
  ...
  if (getMode () == 0) // only draw test options if no option is loaded
    {
      ...
      if (n3ds)
        { // show cStick option is n3ds is enabled.
          m_useColor (0x22, 0x27, 0x2E);
          m_rect (0, 153, 153, 178);
          m_useColor (0x37, 0x67, 0x70);
          m_rect (0, 150, 150, 175);
          C2D_DrawText (&texts[5], C2D_WithColor, 4, 154, 0, 0.5f, 0.5f,
                        white);
        }
    }
    ...
    {
      checkButtons (n3ds);
      C2D_DrawText (&texts[8], C2D_WithColor | C2D_AtBaseline, 2, 236, 0, 0.6f,
                    0.6f, white); // show return text

      C2D_DrawText (&buttons[0], C2D_WithColor, 330, 112, 0, 0.5f, 0.5f,
                    white); // ABXY, DPAD-UDLR
      C2D_DrawText (&buttons[1], C2D_WithColor, 300, 132, 0, 0.5f, 0.5f,
                    white);
      C2D_DrawText (&buttons[2], C2D_WithColor, 300, 92, 0, 0.5f, 0.5f, white);
      C2D_DrawText (&buttons[3], C2D_WithColor, 270, 112, 0, 0.5f, 0.5f,
                    white);
      ...
      C2D_DrawText (&buttons[7], C2D_WithColor, 110, 112, 0, 0.5f, 0.5f,
                    white);
      C2D_DrawText (&buttons[11], C2D_WithColor, 245, 192, 0, 0.5f, 0.5f,
                    white); // START/SELECT
      C2D_DrawText (&buttons[10], C2D_WithColor, 105, 192, 0, 0.5f, 0.5f,
                    white);
      C2D_DrawText (&buttons[8], C2D_WithColor, 55, 52, 0, 0.5f, 0.5f,
                    white); // L/R
      C2D_DrawText (&buttons[9], C2D_WithColor, 295, 52, 0, 0.5f, 0.5f, white);
      if (n3ds)
        { // enable zl/zr if n3ds is true
          C2D_DrawText (&buttons[12], C2D_WithColor, 125, 52, 0, 0.5f, 0.5f,
                        white); // ZL/ZR
          C2D_DrawText (&buttons[13], C2D_WithColor, 255, 52, 0, 0.5f, 0.5f,
                        white);
        }
    }
    ...
    {
      rgbsTop ();
      C2D_DrawText (&texts[7], C2D_WithColor | C2D_AtBaseline, 2, 236, 0, 0.6f,
                    0.6f, white);
      C2D_DrawText (&enabled3D,
                    C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter, 202.5f,
                    120, 0, 1.0f, 1.0f, white);
    }
  else if (getMode () == 4)
    { // Circle pad test
      drawLStick (pxx, pyy);
...
static void
sceneRenderTop3D ()
{ // for the screen test, only enabled when 3D is on
  u32 white = C2D_Color32 (0xFF, 0xFF, 0xFF, 0xFF);
  if (getMode () == 2)
    {
      rgbsTop ();
      C2D_DrawText (&enabled3D,
                    C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter, 197.5f,
                    120, 0, 1.0f, 1.0f, white);
    }
}

static void
sceneRenderBottom () // render common bottom screen elements
{
  adv_background (0x35, 0x3E, 0x4A);
...
int
main () // main stuff, this is where it gets messy
{
  // init stuff
  ...
  while (aptMainLoop ()) // loops forever (until quit[break])
    {
      ...
      if (getMode () == 1
          || getMode ()
                 == 2) // i think this is to control what the bottom screen
                       // shows. it can and will be heavily improved.
        {
          sceneRenderBottom ();
        }
      else if (getMode () == 0)
        {
          ...
          // m_rect(5, 35, 315, 60);
          u32 white = C2D_Color32 (0xFF, 0xFF, 0xFF, 0xFF);
          C2D_DrawText (&texts[9],
                        C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter, 160,
                        23, 0, 0.5f, 0.5f, white);

          // Accessibility. Disabled in mode 0.
      ...
      else if (getMode () == 5)
        {
          ...
          m_rect (5, 85, 315, 110);
          C2D_DrawText (&texts[10],
                        C2D_WithColor | C2D_AtBaseline | C2D_AlignCenter, 160,
                        103, 0, 0.5f, 0.5f, white);
      ...
      else if (getMode () == 7)
        {
          ...
          C2D_DrawText(&detailsView[0], C2D_WithColor, 5, 45, 0, 0.5f, 0.5f,
          white);*/
        }
      else
        {
